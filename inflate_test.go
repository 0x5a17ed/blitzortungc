...
func TestInflate(t *testing.T) {
	tests := []struct {
		...
		{"alphabet", "6162636465666768696a6b6c6d6e6f707172737475767778797a", "abcdefghijklmnopqrstuvwxyz"},
		{"single_multibyte", "c3a4", "ä"},
		{"a_repeated", "61c480c481c480", "aaaaaaaa"},
		{"ab_runs", "61c4806162c48362c481c482c48462", "aaaabbbbaaaabbbbb"},
		{"tobeornot", "544f42454f524e4f54c480c482c484c489c483c485c48723", "TOBEORNOTTOBEORTOBEORNOT#"},

		{"lorem_short", "4c6f72656d2069707375c484646f6cc4812073697420616d65742c20636f6e73" +
		...
		{"lorem_long", "4475697320616c697175657420626c616e6469c48b617263752075c48b677261" +
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := Inflate(data)
			requirepkg.NoError(t, err)
			assertpkg.Equal(t, tt.want, string(got))
		})
	}
}
...
func TestInflate_InvalidUTF8(t *testing.T) {
	cases := map[string][]byte{
		"lone_continuation": {0x80},                   // continuation byte with no leading
		"truncated_2byte":   {0xC4},                   // start of 2-byte sequence, truncated
		"truncated_3byte":   {0xE2, 0x82},             // start of 3-byte sequence, truncated
		"valid_then_bad":    {0x61, 0x62, 0xC4, 0x80, 0xFF}, // valid prefix, invalid tail
	}
